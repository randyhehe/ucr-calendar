...
function CalendarController($scope, $cookies, $window, UserService, CalendarEventService, FriendService, $mdDialog, $mdToast) {
    ...
    function render() {
     ...
     for ( i = 0; i < 42; ++i ){ //clear masterEvent
     ...
     var lastMonth = $scope.currMoment.subtract(1,'months').endOf('month').format('DD'); //returns the number of days in the last month
     ...
     let token = $cookies.get('token');
       ...
       for (i = 0; i < firstDay; i++) { //prev month
        ...
        //let day = moment(prevMonth + '/' + prevMonthDay - 1 + '/', "DD-MM");
        //console.log(day);
        //have to redeclare object everytime to make a 'new' object so
        //we are not override and passing by reference into the array
        var dateObject = {
          month: "month",
          day: 0,
          year: 2017
       ...
       for (i = 1; i <= numberOfDays; i++) { //curr month
         ...
         if (i == $scope.currentDate.format('D') && $scope.currMoment.format("M")
           ...
           currentDayIndex = tempFirstDay - 1; //find the index of the current day out of 42 boxes
        ...
        for (j = 0; j < res.data.events.length; ++j) {//check if event is on that day
         ...
         dayCnt++
         //console.log(i);
         //console.log($scope.currMoment.format('M'));
         //have to redeclare object everytime to make a 'new' object so
         //we are not override and passing by reference into the array
         var dateObject = {
           month: "month",
